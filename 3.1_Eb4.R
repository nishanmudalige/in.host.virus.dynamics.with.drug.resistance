# Same as Eb1 with drug resistance
...
for (j in 1:(nt-1) ){
  Tr[j+1] = Tr[j] + h*( lambda - delta*Tr[j] - ( (1-epsilon1)*(1-epsilon2)*bs*Is[j] + br*Ir[j])*Tr[j]  )
  
  Is[j+1] = Is[j] + h*( (1-epsilon1)*(1-epsilon2)*(1-mu)*bs*Is[j]*Tr[j] - a*Is[j] )
  Ir[j+1] = Ir[j] + h*( br*Tr[j]*Ir[j] - a*Ir[j] + (1-epsilon1)*(1-epsilon2)*mu*bs*Is[j]*Tr[j]  )
